...
function syncRelationships() {
  ...
  for (const sKey in SERVICES) {
    for (const pKey of SERVICES[sKey].participants || []) {
  ...
  try {
    ...
    try {
      ...
      if (textData.includes('<html') || textData.includes('<!DOCTYPE')) {
        throw new Error(
          'O Script retornou HTML em vez de JSON. Verifique se a URL da API está correta e se o script foi implantado como Aplicativo da Web acessível a qualquer pessoa.',
        );
      }
      throw new Error('Erro ao processar dados (JSON inválido).');
    }

    if (!isValidPaidLogsResponse(data)) {
      console.warn('Resposta do servidor ignorada por formato inválido:', data);
      throw new Error(
        'Os dados recebidos do servidor não estão no formato esperado.',
      );
    }
    ...
    try {
      const configRes = await fetch(
        `${urlWithCacheBuster}&sheetName=Configuracoes`,
        {
          redirect: 'follow',
          cache: 'no-store',
        },
    ...
    Object.entries(configData).forEach(([person, logs]) => {
      if (metadataKeys.includes(person)) return;
      if (typeof logs === 'object' && logs !== null && !Array.isArray(logs)) {
        mergedData[person] = { ...(mergedData[person] || {}), ...logs };
      }
    });

    // Normalizar dados remotos (formato simples: presença = pago) e sobrescrever local
    const remoteLogs = normalizePaidLogs(mergedData);
    paidLogsCache = remoteLogs;

    savePaidLogs(paidLogsCache);
    updateUIAfterSync();
  } catch (error) {
    console.error(
      `Tentativa ${retryCount + 1} de sincronização falhou:`,
      error,
    );
    ...
    // Fallback para dados locais se tudo falhar
    paidLogsCache = getPaidLogs();
    updateUIAfterSync();

    let userMessage = 'Modo offline: usando dados salvos no aparelho.';
    if (error.message.includes('HTML')) {
      userMessage =
        'Erro na URL do Script: Verifique se a implantação no Apps Script está correta.';
    } else if (error.message.includes('servidor não estão')) {
      userMessage =
        'Erro de dados: O servidor retornou uma resposta inesperada.';
    } else if (
      ...
      userMessage =
        'Falha de conexão: Verifique sua internet, adblockers ou se o Script está publicado como "Qualquer pessoa" (não apenas usuários do Google).';
    }

    setNotificationStatus(userMessage, true);
  }
}
...
async function markPaymentAsPaid(personKey, payment) {
  ...
  try {
    ...
    setNotificationStatus(
      'Erro de conexão ao salvar. O dado está salvo localmente e tentará sincronizar depois.',
      true,
    );
  }
}
...
function handleAdminSave() {
  if (currentAdminContext.type === 'service') {
    ...
    const aliases = [name.split(' ')[0].toLowerCase()];
...
async function saveGlobalConfig() {
  ...
  localStorage.setItem('site_settings', jsonStr);
  if (!API_URL || API_URL.includes('COLA_TUA_URL')) return;

  const adminKey = 'admin';
  paidLogsCache[adminKey] = paidLogsCache[adminKey] || {};

  try {
    setNotificationStatus('Salvando configurações globais...');

    // Usar timestamp no GID para que o sort().reverse() encontre a versão mais recente
    const gid = Date.now().toString(36) + '_' + generateKey();
    const encoded = toBase64Unicode(jsonStr);
    const SINGLE_THRESHOLD = 3000; // base64 chars threshold for single write

    if (encoded.length <= SINGLE_THRESHOLD) {
      const mesValue = `site_settings|${gid}|${jsonStr}`;
      // update local cache immediately
      paidLogsCache[adminKey][`config:${mesValue}`] = gid;
      savePaidLogs(paidLogsCache);
      ...
      setNotificationStatus('Configurações salvas para todos!');
      return;
    }

    // Chunked upload fallback for large configs
    const CHUNK_SIZE = 2500; // base64 chars per chunk
    const total = Math.ceil(encoded.length / CHUNK_SIZE);

    for (let i = 0; i < total; i += 1) {
      const chunk = encoded.slice(i * CHUNK_SIZE, (i + 1) * CHUNK_SIZE);
      const mesValue = `site_settings_chunk|${gid}|${i}|${total}|${chunk}`;
      paidLogsCache[adminKey][`config:${mesValue}`] = gid;
      savePaidLogs(paidLogsCache);

      // best-effort POST for each chunk
      await fetch(API_URL, {
        method: 'POST',
        mode: 'no-cors',
        referrerPolicy: 'no-referrer',
    ...
    setNotificationStatus('Configurações salvas (fragmentadas) para todos!');
  } catch (err) {
    console.error(err);
    setNotificationStatus(
      'Erro. Configurações foram salvas apenas localmente.',
    );
  }
}
