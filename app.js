...
function bindEvents() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (event.target === invoiceModal) invoiceModal.classList.add('is-hidden');
  ...
  window.addEventListener('focus', () => {
    // Só sincroniza e atualiza o dashboard se um perfil já estiver selecionado
    if (state.currentPersonKey) {
      // Se o modal de senha estiver aberto, não disparar sincronização em background
      // para não competir por recursos com a verificação de senha.
      if (
        !document
          .querySelector('#passwordModal')
...
let currentHintText = '';
let passwordAuthAttempt = 0; // Contador para invalidar requisições antigas
...
async function handleProfileClick(personKey) {
  ...
  const currentAttempt = ++passwordAuthAttempt;
  passwordSubmitBtn.disabled = false; // Garante que o botão comece habilitado
  ...
  try {
    ...
    // Se houve uma nova tentativa de login ou o modal foi fechado, descarta esta resposta
    if (
      currentAttempt !== passwordAuthAttempt ||
      passwordModal.classList.contains('is-hidden')
    )
      return;
  ...
  if (btnDeleteGroup) {
    ...
    btnDeleteGroup.addEventListener('click', async () => {
      ...
      try {
        ...
        renameGroupMessage.textContent =
          'Erro ao excluir: ' + (e.message || 'conexão com o banco falhou.');
...
async function carregarListaDeGrupos() {
  ...
  const renderGroupListLocal = (grupos) => {
    ...
    if (grupos.length === 0) {
      groupList.innerHTML =
        '<p style="text-align: center; color: var(--muted); font-size: 0.9rem; grid-column: 1/-1;">Nenhum grupo encontrado.</p>';
    } else {
      const colors = [
        '#dc2626',
        '#16a34a',
  ...
  const cachedGroupsStr = localStorage.getItem(
    'streaming-payments-groups-cache',
  );
  if (cachedGroupsStr) {
    try {
      ...
      groupList.innerHTML =
        '<div style="grid-column: 1/-1; display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 20px 0; width: 100%;"><div class="loader-spinner"></div><p style="color: var(--muted); font-size: 0.9rem; margin: 0;">Buscando grupos...</p></div>';
    }
  } else {
    groupList.innerHTML =
      '<div style="grid-column: 1/-1; display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 20px 0; width: 100%;"><div class="loader-spinner"></div><p style="color: var(--muted); font-size: 0.9rem; margin: 0;">Buscando grupos...</p></div>';
  ...
  try {
    ...
    if (data.success && data.grupos) {
      ...
      groupList.innerHTML =
        '<p style="text-align: center; color: var(--danger); font-size: 0.9rem; grid-column: 1/-1;">Erro ao carregar os grupos.</p>';
    }
  } catch (error) {
    if (!cachedGroupsStr) {
      groupList.innerHTML =
        '<p style="text-align: center; color: var(--danger); font-size: 0.9rem; grid-column: 1/-1;">Erro de conexão.</p>';
    }
  }
}
